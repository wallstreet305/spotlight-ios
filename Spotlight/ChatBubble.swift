...
class ChatBubble: UIView {
    ...
    */
    ...
    /**
     ...
     :returns: Chat Bubble
     */
    init(data: ChatBubbleData, startX:CGFloat , startY: CGFloat){
        ...
        super.init(frame: ChatBubble.framePrimary(data.type, startY:startY, screenSize: ScreenSize))
        ...
        if let chatImage = data.link {
            
            let width: CGFloat = 150//min(chatImage.size.width, CGRectGetWidth(self.frame) - 2 * padding)
            let height: CGFloat = 150 //chatImage.size.height * (width / chatImage.size.width)
            imageViewChat = UIImageView(frame: CGRectMake(padding, padding, width, height))
            //imageViewChat?.image = chatImage
            imageViewChat?.contentMode = UIViewContentMode.ScaleAspectFit
            
            imageViewChat?.loadImageFromURL(NSURL(string: chatImage)!, placeholderImage: UIImage(named: "text"))
            imageViewChat!.tag = 123
        ...
        if let chatText = data.text {
            ...
            labelChatText?.font = UIFont.systemFontOfSize(17)
    ...
    class func framePrimary(type:BubbleDataType, startY: CGFloat, screenSize: CGRect) -> CGRect{
